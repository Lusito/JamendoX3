...
package info.lusito.jamendo.android.widgets.dragdrop;
 ...
 *
 * @author Google
 * @author Santo Pfingsten (some modifications)
 * @see http://www.youtube.com/watch?v=_BZIvjMgH-Q
 */
public class DragDropListView extends ListView implements AdapterView.OnItemLongClickListener {

    private final int LEFT_OFFSET = 8;
    ...
    private DragDropSwapper swapper;
    ...
    public void setSwapper(DragDropSwapper swapper) {
    ...
    public boolean onItemLongClick(AdapterView<?> arg0, View arg1, int pos, long id) {
        if(swapper == null)
            throw new IllegalStateException("Swapper has not been set!");
    ...
    private BitmapDrawable getAndAddHoverView(View v) {
        ...
        mHoverCellCurrentBounds.left += LEFT_OFFSET;
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (mHoverCell != null) {
            Rect oldBounds = canvas.getClipBounds();
            canvas.clipRect(mHoverCellCurrentBounds);
            getBackground().draw(canvas);
            mHoverCell.draw(canvas);
            mHoverCell.setAlpha(mHoverCellAlpha);
            canvas.clipRect(oldBounds);
        }
    }
    ...
    public boolean onTouchEvent (MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mCellIsMobile) {
                    double pct = 1.0 - (Math.abs(deltaX) / (mHoverCellOriginalBounds.width()/3.0));
                    mHoverCellAlpha = Math.max(0, Math.min(255, (int)(255 * pct)));
                    mHoverCellCurrentBounds.offsetTo(mHoverCellOriginalBounds.left + LEFT_OFFSET + deltaX,
    ...
    private void handleCellSwitch() {
        ...
        int deltaYTotalCenter = deltaYTotal + mHoverCellCurrentBounds.height()/2;

        View belowView = getViewForID(mBelowItemId);
        View mobileView = getViewForID(mMobileItemId);
        View aboveView = getViewForID(mAboveItemId);

        boolean isBelow = (belowView != null) && (deltaYTotalCenter > belowView.getTop());
        boolean isAbove = (aboveView != null) && (deltaYTotalCenter < aboveView.getBottom());
        ...
        if (isBelow || isAbove) {
            ...
            swapper.swapEntries(mobileView, switchView);
    ...
    private void touchEventsEnded () {
        ...
        if (mCellIsMobile|| mIsWaitingForScrollFinish) {
            ...
            if(mHoverCellAlpha == 0) {
                swapper.removeEntry(mobileView);
            ...
            hoverViewAnimator.addUpdateListener((ValueAnimator va)->invalidate());
